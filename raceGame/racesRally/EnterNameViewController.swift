...
class EnterNameViewController: UIViewController {
    ...
    @IBAction func saveResult(_ sender: Any) {
            if let data = UserDefaults.standard.value(forKey: "gamerInfo") as? Data {
                do {
                    gamersResult = try decoder.decode([Player].self, from: data)
                } catch {
            ...
            if let points = UserDefaults.standard.value(forKey: "myPoints") as? Int {
                ...
                let gamer = Player(name: nameTextField.text!, seconds: points, time: dateFormatter.string(from: Date()))
    ...
    private func showShareAlert() {
        ...
        let alert = UIAlertController(title: "Share a result on Facebook", message: "Do you want to share your result?", preferredStyle: .alert)
        ...
        if let points = UserDefaults.standard.value(forKey: "myPoints") as? Int {
            alert.addAction(UIAlertAction(title: "Share", style: .default, handler: { (action) in
                ...
                guard let tweetURL = URL(string: "https://Facebook.com/intent/tweet?text=\(encodedText)") else {
                return }
